...
from .ir_codes import CARRIER_FREQUENCY, IR_CODES, REPEAT_OFFSET, REPEAT_COUNT
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    _LOGGER.debug(f"setup sony with config->{config}")
    ...
    if gc100 is None:
        ...
        return
    async_add_entities([ SonyRemote(host, port, gc100, gc_addr, name) ])



class SonyRemote(remote.RemoteDevice):
    """Home Assistant instance of a Sony Ultra HD BluRay player, IR control via GC-100"""
    ...
    def __init__(self, host, port, gc100, gc_addr, name):
        ...
        self.commands ={cmd : self.gc100_ir.format_sendir(freq=CARRIER_FREQUENCY, offset=REPEAT_OFFSET, count=REPEAT_COUNT, code=onoff) for cmd, onoff in IR_CODES.items()}
    ...
    async def async_turn_on(self, **kwargs):
        ...
        try:
            r = await self.gc100_ir.sendir_raw(self.commands['POWER_ON'])
            _LOGGER.debug(f"turn_on ->{r}")
    ...
    async def async_turn_off(self, **kwargs):
        ...
        try:
            r = await self.gc100_ir.sendir_raw(self.commands['POWER_OFF'])
            _LOGGER.debug(f"turn_off ->{r}")
        except Exception as e:
            _LOGGER.error(e)
    ...
    async def async_send_command(self, command, **kwargs):
        ...
        for com in command:
            try:
                ...
                _LOGGER.debug(f"send_command ({com})->{r}")
            except Exception as e:
                _LOGGER.error(e)
