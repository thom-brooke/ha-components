...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    _LOGGER.debug(f"setup panasonic with config->{config}")
    ...
    if gc100 is None:
        ...
        return
    async_add_entities([ PanasonicRemote(host, port, gc100, gc_addr, name) ])



class PanasonicRemote(remote.RemoteDevice):
    """Home Assistant instance of a Panasonic BluRay player, IR control via GC-100"""
